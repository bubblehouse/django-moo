...
- `translator.py` — IR → Python verb-source translation. Per-routine, per-clause, per-M-clause splits live here.
- `generator.py` — drives translator output into a complete `moo/bootstrap/zork1/` tree (rooms, objects, exits, tables, verbs).
- `verbs/` — static templates copied verbatim into generated `verbs/`:
  - `verbs/zil_sdk/` — runtime impedance shims (flag/zstate/table primitives) the translator emits calls to.
  - `verbs/system/`, `verbs/PREFIX.py`, `verbs/SUFFIX.py` — System Object verbs and delimiter helpers.
