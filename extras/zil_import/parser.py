...
_TOKEN_RE = re.compile(
    # The string body uses non-overlapping branches: ``[^"\\]`` matches any
    # char that's not a quote or backslash, and ``\\.`` matches an escape
    # sequence.  Any overlap (e.g. an extra ``[^"]`` branch) makes the regex
    # catastrophically backtrack on long no-string passages — substrate's
    # syntax.zil hung the importer for >5 minutes before this fix.
    r'(?P<string>"(?:[^"\\]|\\.)*")'
    r"|"
    # ZIL uses backslash to escape any character so it's read literally, e.g.
    # ``<BUZZ \, \" \. >`` lists the comma/quote/period as buzzwords.  Match
    # ``\<char>`` as a single atom token; without this, the bare ``"`` after
    # ``\`` would start a string that runs to EOF.
    r"(?P<escape>\\.)"
    r"|"
    r"(?P<nil><>)"
    r"|"
    ...
    r"(?P<semicolon>;)"
    r"|"
    # Number must NOT be followed by an atom continuation char — otherwise
    # ZIL idioms like ``0?`` (zero predicate) and ``1?`` (one predicate)
    # split as a number plus a separate ``?`` atom.
