...
See ``extras/zil_import/ARCHITECTURE.md`` for the cleanup plan.
"""
...
def test_no_new_zmachine_primitive_leakage() -> None:
    ...
    if new_leaks:
        ...
        msg.append(
            ...
            "(only if you've documented the gap in ARCHITECTURE.md)."
        )
        pytest.fail("\n".join(msg))
...
def test_total_primitive_count_does_not_grow() -> None:
    ...
    assert total <= BASELINE, (
        ...
        f"raise the BASELINE in this test (and document why in ARCHITECTURE.md)."
    )
