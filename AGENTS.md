...
| Verb | File | Behaviour |
|------|------|-----------|
| `@mail` | `player/at_mail.py` | list, read, delete, undelete, stats |
| `@send <player>` | `player/at_send.py` | compose via editor; first line = subject |
| `@reply <n>` | `player/at_reply.py` | editor pre-filled with quoted body, `Re:` subject |
| `@forward <n> to <player>` | `player/at_mail_forward.py` | forward with `Fwd:` subject |
| `@gripe` | `player/at_gripe.py` | sends to `_.gripe_recipients` on the system object |
| `confunc` | `player/confunc.py` | shows unread count on login via `count_unread` |
...
9. Push and create merge request targeting `main`
10. Address review feedback
11. After merge, automated CI/CD handles testing and deployment
