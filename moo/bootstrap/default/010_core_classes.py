...
flashlight, _ = bootstrap.get_or_create_object("Generic Flashlight", unique_name=True, parents=[thing])
sys.set_property("flashlight", flashlight)
flashlight.set_property("description", "A small battery-powered flashlight with a single sliding switch.")

daemon, _ = bootstrap.get_or_create_object("Generic Daemon", unique_name=True, parents=[thing])
sys.set_property("daemon", daemon)
daemon.set_property("description", "An invisible thing that fires a verb on a schedule.")
daemon.set_property("interval", 60, inherit_owner=True)
daemon.set_property("periodic_task_id", None, inherit_owner=True)
