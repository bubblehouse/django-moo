#!moo verb turnfunc --on "Gas Room" --dspec either
# Generated by extras/zil_import — do not edit by hand
# pylint: disable=return-outside-function,undefined-variable,pointless-statement,unnecessary-negation,disallowed-name,using-constant-test,redefined-outer-name,no-else-return,unused-variable,redefined-builtin,singleton-comparison,unnecessary-pass,expression-not-assigned
from moo.sdk import context, lookup
# ZIL: BOOM-ROOM / M-END

parser = context.parser
candles = lookup("candles")
match = lookup("match")
torch = lookup("torch")
rarg = args[0] if len(args) > 0 else "M-END"
dummy_p = 0
the_player_verb = (
    args[1]
    if len(args) > 1
    else (parser.words[0].lower() if context.parser is not None and parser.words else verb_name)
)

if rarg == "M-END" and the_player_verb in ["lamp-on", "burn", "light"] and parser.get_dobj() in (candles, torch, match):
    dummy_p = True
if (
    _.zork_thing.is_held(candles)
    and candles.flag("onbit")
    or _.zork_thing.is_held(torch)
    and torch.flag("onbit")
    or _.zork_thing.is_held(match)
    and match.flag("onbit")
):
    if dummy_p:
        print(
            "How sad for an aspiring adventurer to light a "
            + parser.get_dobj().desc()
            + " in a room which\nreeks of gas. Fortunately, there is justice in the world."
        )
    else:
        print(
            "Oh dear. It appears that the smell coming from this room was coal gas.\nI would have thought twice about carrying flaming objects in here."
        )
    _.jigs_up("\n\n      ** BOOOOOOOOOOOM **")
