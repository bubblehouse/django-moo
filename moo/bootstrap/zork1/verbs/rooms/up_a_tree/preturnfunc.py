...
if the_player_verb == "climb-down" and parser.get_dobj() in (tree, player.zstate_get("ROOMS")):
    _.walk(player.zstate_get("P?DOWN"))
elif the_player_verb in ["climb-up", "climb-foo"] and parser.get_dobj() == tree:
    _.walk(player.zstate_get("P?UP"))
elif the_player_verb == "drop":
    if not _.zork_thing.idrop():
        return True
    elif parser.get_dobj() == nest and egg.location == nest:
        print("The nest falls to the ground, and the egg spills out of it, seriously\ndamaged.")
        _.remove(egg)
        lookup("broken_egg").moveto(path)
    elif parser.get_dobj() == egg:
        print("The egg falls to the ground and springs open, seriously damaged.", end="")
        egg.moveto(path)
        # ZIL: <BAD-EGG ...>
        _.zork_thing.bad_egg()
        print()
    elif not parser.get_dobj() in (player, tree):
        parser.get_dobj().moveto(path)
        print("The " + parser.get_dobj().desc() + " falls to the ground.")
