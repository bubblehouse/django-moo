...
- `zork1/`: Bootstrap package for the example Zork 1 dataset. Generated by `extras/zil_import` from upstream MIT-licensed ZIL source; checked-in output, not regenerated in normal use.
...
- **zork1**: An example bootstrap derived from the open-source Zork 1 ZIL source via `extras/zil_import`. Demonstrates the from-scratch `custom-world` package shape with a parallel root-class hierarchy (`Zork Root`/`Zork Room`/`Zork Thing`/`Zork Container`/`Zork Actor`).
