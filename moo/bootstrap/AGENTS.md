...
> # 🛑 STOP — THE LOADER AND `default/` ARE GENERIC INFRASTRUCTURE
>
> ## **DO NOT EDIT THE BOOTSTRAP LOADER OR DEFAULT VERBS TO ACCOMMODATE A SPECIFIC DATASET.**
>
> The shared bootstrap machinery (`moo/bootstrap/__init__.py`, `parse_shebang`, `load_verbs`, `get_or_create_object`) is generic infrastructure used by every dataset, including out-of-tree datasets that contribute through the `moo.bootstrap.*` namespace package. Adding dataset-driven behavior to it is forbidden.
>
> The `default/` dataset is the canonical example world for new operators. Don't change its verb shebangs, dspecs, or contents to make a foreign dataset's verbs dispatch correctly.
...
- `default/`: Bootstrap package for the `default` game world. `bootstrap.py` orchestrates numbered scripts (`010_core_classes.py`, `020_utility_objects.py`, ...) executed in sorted order; `__init__.py` is empty so importing the package for test discovery does not run setup
- `test.py`: Flat module providing the `test` dataset used by core pytest tests
- `default/verbs/`: Verb files installed on `default` game objects, organised by root-class name
- `default/tests/`: Pytest integration tests for the verbs and bootstrap output

Out-of-tree datasets contribute their own packages under `moo.bootstrap.*` via PEP 420 namespace packages. The companion moo-agent project ships one — `moo.bootstrap.zork1`, an example derived from the Infocom *Zork I* MIT source — which becomes loadable through `moo_init --bootstrap zork1` when moo-agent is installed alongside django-moo.
...
- `moo/bootstrap/default/tests/` — pytest integration tests for the verbs installed on the `default` dataset. These are the tests to write when adding or changing a `default/verbs/` verb.
...
### Bootstrap integration tests (`default/tests/`)

Tests for verbs in `default/verbs/` live in `default/tests/` and run against the fully bootstrapped `default` world. Two fixtures from `moo/conftest.py` are required:
...
Verbs in `default/verbs/` are loaded by `bootstrap.load_verbs()` at the end of `default.py`:
...
The reference implementation is `moo/bootstrap/default/verbs/programmer/at_reload.py`.
...
Do: Create verb file in `default/verbs/`
...
Verbs on `$player` allow automation clients and accessibility-conscious users to control how command output is delivered. They are implemented in `default/verbs/player/` and use the `set_session_setting` / `get_session_setting` SDK functions.
...
- See `default/verbs/` for example verbs in this repository
