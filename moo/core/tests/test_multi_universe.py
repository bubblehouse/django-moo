...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_object_save_fills_site_from_context(t_two_sites, t_wizard):
    """A new Object inherits site from the active context, not the caller."""
    _, site2 = t_two_sites
    with code.ContextManager(t_wizard, lambda _: None, site=site2):
        obj = Object.global_objects.create(name="ctx-spawned", owner=t_wizard)
    assert obj.site_id == site2.pk


@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_object_save_fills_site_from_caller(t_two_sites):
    """When context lacks site but caller has one, save inherits caller.site."""
    site1, site2 = t_two_sites
    site2_wizard = Object.global_objects.get(name="Wizard", site=site2)
    # Context has no site; caller is on site2.
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_object_save_fills_site_from_default(t_init):
    """No context and no caller: save falls back to SITE_ID."""
    obj = Object.global_objects.create(name="defaulted")
    assert obj.site_id == get_default_site().pk
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_universal_wizard_auto_provisions_on_new_site(t_two_sites):
    """A UniversalWizard user gets a wizard avatar+Player on first connect to site2."""
    _, site2 = t_two_sites
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_universal_wizard_idempotent(t_two_sites):
    """Re-running auto-provision does not create a duplicate Player or avatar."""
    _, site2 = t_two_sites
    user = User.objects.create_user(username="phil", password="x")
    UniversalWizard.objects.create(user=user)
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_resolve_site_maps_hostname_to_site(t_two_sites):
    """``_resolve_site`` matches the connection's server_host to a Site domain."""
    _, site2 = t_two_sites
    conn = MagicMock()
    conn.get_extra_info.return_value = "site2.test"

    server = _ssh_server_with_user(user=None, site=None, conn=conn)
    server._resolve_site()  # pylint: disable=protected-access
    assert server.site.pk == site2.pk


@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_resolve_site_falls_back_to_default(t_two_sites):
    """An unknown hostname falls back to the default Site."""
    site1, _ = t_two_sites
    conn = MagicMock()
    conn.get_extra_info.return_value = "unknown.test"

    server = _ssh_server_with_user(user=None, site=None, conn=conn)
    server._resolve_site()  # pylint: disable=protected-access
    assert server.site.pk == site1.pk
